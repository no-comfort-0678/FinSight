 ...
 * ADDED: Net Transactions (Debt Minimization) feature.
 * Rules: Providing the whole code after changes [2026-03-29].
 */
import React, { useState, useEffect, useMemo } from "react";
import { 
  History, RefreshCw, ChevronLeft, DollarSign, 
  CheckCircle2, X, Users, PieChart, Banknote, AlertTriangle, RotateCcw, Save, Trash2, PlusCircle, Download, ArrowRightLeft
} from "lucide-react";
...
  const [showSummaryModal, setShowSummaryModal] = useState(false);
  const [showNetModal, setShowNetModal] = useState(false); // New state for Net Transactions
...
    }
  ...
  // --- NEW: FETCH MINIMIZED NET TRANSACTIONS ---
  const fetchNetTransactions = async () => {
    try {
      const res = await fetch(`${API}/net-transactions/${activeRoom.id}`, {
        headers: { "Authorization": `Bearer ${localStorage.getItem("token")}` }
      });
      const data = await res.json();
      setSummaryData(data);
      setShowNetModal(true);
    } catch (err) { alert("Failed to load net transactions"); }
  };
...
        alert(`Settled all debts with ${targetUsername}!`);
        if (showNetModal) fetchNetTransactions(); else fetchSummary();
        getHistory(); 
      }
...
          <div className="header-utility" style={{display:'flex', gap: '12px', alignItems: 'center'}}>
            <button className="btn-yellow-outline" style={{padding: '8px 12px', fontSize: '12px', width:'auto', border: '1px solid #00ff88', color: '#00ff88'}} onClick={fetchNetTransactions}>
              <ArrowRightLeft size={16} style={{marginRight: '6px'}}/> NET TRANSACTIONS
            </button>
...
{/* --- REVERTED SUMMARY MODAL (Original Style) --- */}
     {showSummaryModal && (
  <div className="split-modal-overlay">
            ...
            <tbody>
              {summaryData.map((item, idx) => (
                <tr key={idx}>
                  ...
                  <td id="green-amount"> ${item.oweToYou}</td>
                  <td id="red-amount">${item.youOweThem}</td>
                  <td>
                    {parseFloat(item.oweToYou) > 0 && (
                      <button className="btn-yellow-mini" onClick={() => handleSettleDebt(item.username)}>
                        SETTLE
                      </button>
                    )}
                  </td>
...
{/* --- NEW MODAL: NET TRANSACTIONS (MINIMIZED) --- */}
{showNetModal && (
  <div className="split-modal-overlay">
    <div className="split-modal-box" style={{maxWidth: '600px'}}>
      <div className="modal-top card-header-row">
        <h3 className="yellow-text">FINAL SETTLEMENTS</h3>
        <X className="cursor-p" onClick={() => setShowNetModal(false)} size={24} />
      </div>
      <div className="summary-list-v3">
          <table className="history-table-v3">
            <thead><tr><th>FROM</th><th>TO</th><th>AMOUNT</th><th>ACTION</th></tr></thead>
            <tbody>
              {summaryData.length > 0 ? summaryData.map((tx, idx) => (
                <tr key={idx}>
                  <td className="yellow-bold">@{tx.from}</td>
                  <td className="yellow-bold">@{tx.to}</td>
                  <td id="green-amount">${parseFloat(tx.amount).toFixed(2)}</td>
                  <td >
                    {tx.from === user.username ? (
                      <button className="btn-yellow-mini" style={{background: '#00ff88', color: '#000'}} onClick={() => handleSettleDebt(tx.to)}>
                        <Banknote size={14}/> SETTLE
                      </button>
                    ) : (
                      <small id="red-amount">Owed by @{ tx.from}</small>
                    )}
                  </td>
                </tr>
              )) : <tr><td colSpan="4" style={{textAlign:'center', padding:'20px'}}>No pending debts in this room.</td></tr>}
            </tbody>
          </table>
      </div>
      <button className="btn-yellow-solid full-w spacing-v" onClick={() => setShowNetModal(false)}>CLOSE</button>
    </div>
  </div>
)}
    </div>
